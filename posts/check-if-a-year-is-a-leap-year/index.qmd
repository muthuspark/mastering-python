...
Determining whether a year is a leap year is a common programming task, especially when working with dates and calendars.  Python offers straightforward ways to accomplish this, and this post will explore several approaches, from basic conditional logic to using the `calendar` module.
...
For robust code, consider adding error handling to manage invalid input, such as non-integer values or negative years.  This could involve using a `try-except` block to catch `TypeError` or `ValueError` exceptions.  We leave this as an exercise for the reader to explore and enhance the provided examples.
