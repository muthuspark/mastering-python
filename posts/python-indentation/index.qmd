...
Python, unlike many other programming languages, relies heavily on indentation to define code blocks.  This might seem unusual at first, but it significantly contributes to Python's readability and enforces a consistent coding style.  Instead of using curly braces `{}` like in C++, Java, or JavaScript, Python uses whitespace (typically four spaces) to indicate the start and end of code blocks such as loops, functions, and conditional statements.
